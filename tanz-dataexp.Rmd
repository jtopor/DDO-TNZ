...
In fact, analysis of the __amount_tsh__ variable indicates that 41,639 of the 59,400 data records (70.3%) have a value of "0":
...
This leaves slightly less than 3,000 records having __amount_tsh__ values that exceed 1000. Such values should be investigated to determine whether they are in fact valid values for the variable. Of those 3000 records, 2,810 are found to lie within the range of (1000:10,000):
...
Barplots for `amount_tsh` values within the range of (1:10000) suggest that pumps with relatively larger values may be more likely to be `functional` than are wells having smaller values. However, since the larger values might possibly be outliers and we lack valid `amount_tsh` values for more than 70% of the records within the data set, it seems highly unlikely that we can derive any valid predictive inferences from the plots.

```{r, echo = FALSE}
tot_atsh <- hist(tanz$amount_tsh[tanz$amount_tsh > 0 & tanz$amount_tsh <= 10000] , breaks = 30, plot = FALSE)
atsh_func <- hist(tanz$amount_tsh[tanz$amount_tsh > 0 & tanz$amount_tsh <= 10000 & tanz$label == 'functional'], breaks = 30, plot = FALSE )

barplot(atsh_func$counts / tot_atsh$counts,
        names.arg = atsh_func$breaks[2:length(atsh_func$breaks)],
        main = ('% of Wells Functional: 0 < amount_tsh <= 10000'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "amount_tsh (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )

# add a line indicating overall functional percentage
abline(h = functional[[1]], lty = 2, lwd = 1.5, col = "blue")
```


```{r, echo = FALSE}
# amount_tsh vs non functional

atsh_nfunc <- hist(tanz$amount_tsh[tanz$amount_tsh > 0 & tanz$amount_tsh <= 10000 & tanz$label == 'non functional'], plot = FALSE, breaks = 30 )

barplot(atsh_nfunc$counts / tot_atsh$counts, 
        names.arg = atsh_nfunc$breaks[2:length(atsh_nfunc$breaks)],
        main = ('% of Wells Non Functional: 0 < amount_tsh <= 10000'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "amount_tsh (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )

# add a line indicating overall non-functional percentage
abline(h = n_func[[1]], lty = 2, lwd = 1.5, col = "red")

# memory cleanup
rm(atsh_func, atsh_nfunc, tot_atsh)
```

Given the large percentage of missing values for the variable, other variables within the data set were examined to determine whether any of their attributes might serve as indicators or proxies for missing  `amount_tsh` values. The __extraction_type_group__ and __waterpoint_type_group__ categorical variables are each at least somewhat indicative of the type of well deployed at each waterpoint. Assuming similar types of wells generally share somewhat similar physical characteristics and limitations, there may be a relationship we can infer between these two variables and the __amount_tsh__ value each well may reasonably have. The tables below summarize the median non-zero __amount_tsh__ values for each category of the __extraction_type_group__ and __waterpoint_type_group__ variables. 
...
Barplots for the non-zero `gps_height` values show that pumps located at relatively higher altitudes are more likely to be `functional` than are pumps found at lower altitudes. However, pumps located at altitudes higher than a`gps_height` value of approximately 2500 are also the most likely to be `functional needs repair`, followed by those lying within the approximate range of (1000:1600). We also find that pumps located at relatively low altitudes (i.e., between 0 and 800) are the most likely to be non-functional.

```{r, echo = FALSE}
# gps_height vs functional

tot_gps <- hist(tanz$gps_height[tanz$gps_height != 0], plot = FALSE )
gps_func <- hist(tanz$gps_height[tanz$gps_height != 0 & tanz$label == 'functional'], plot = FALSE )

barplot(gps_func$counts / tot_gps$counts, 
        names.arg = gps_func$breaks[2:length(gps_func$breaks)],
        main = ('% of Wells Functional: gps_height != 0'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "GPS Height (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# gps_height vs functional needs repair

#tot_gps <- hist(tanz$gps_height[tanz$gps_height != 0], plot = FALSE )
gps_func_nr <- hist(tanz$gps_height[tanz$gps_height != 0 & tanz$label == 'functional needs repair'], plot = FALSE )

barplot(gps_func_nr$counts / tot_gps$counts, 
        names.arg = gps_func_nr$breaks[2:length(gps_func_nr$breaks)],
        main = ('% of Wells Functional Needs Repair: gps_height != 0'), ylim = c(0, 0.2), col =
          'yellow', las = 2, xlab = "GPS Height (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )

# add a line indicating overall func needs repair percentage
abline(h = func_n_rep[[1]], lty = 2, lwd = 1.5, col = "red")
```

```{r, echo = FALSE}
# gps_height vs non functional

gps_nfunc <- hist(tanz$gps_height[tanz$gps_height != 0 & tanz$label == 'non functional'], plot = FALSE )

barplot(gps_nfunc$counts / tot_gps$counts, 
        names.arg = gps_nfunc$breaks[2:length(gps_nfunc$breaks)],
        main = ('% of Wells Non Functional: gps_height != 0'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "GPS Height (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
# memory cleanup
rm(gps_func, gps_func_nr, gps_nfunc, tot_gps)
```
...
tot_pop <- hist(tanz$population[tanz$population > 0 & tanz$population <= 1000], plot = FALSE )
pop_func <- hist(tanz$population[tanz$population > 0 & tanz$population <= 1000 & tanz$label == 'functional'], plot = FALSE )

barplot(pop_func$counts / tot_pop$counts, 
        names.arg = pop_func$breaks[2:length(pop_func$breaks)],
        main = ('% of Wells Functional: 0 < population <= 1000'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Population (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# population vs functional needs repair

pop_func_nr <- hist(tanz$population[tanz$population > 0 & tanz$population <= 1000 & tanz$label == 'functional needs repair'], breaks = 19, plot = FALSE )

barplot(pop_func_nr$counts / tot_pop$counts, 
        names.arg = pop_func_nr$breaks[2:length(pop_func_nr$breaks)],
        main = ('% of Wells Functional Needs Repair: 0 < population <= 1000'), ylim = c(0, .3), col =
          'yellow', las = 2, xlab = "Population (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# population vs non functional

pop_nfunc <- hist(tanz$population[tanz$population > 0 & tanz$population <= 1000 & tanz$label == 'non functional'], plot = FALSE, breaks = 19 )

barplot(pop_nfunc$counts / tot_pop$counts, 
        names.arg = pop_nfunc$breaks[2:length(pop_nfunc$breaks)],
        main = ('% of Wells Non Functional: 0 < population <= 1000'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Population (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
# memory cleanup
rm(pop_func, pop_func_nr, pop_nfunc, tot_pop)
```
...
```{r, echo = FALSE}
# longitude vs functional

tot_long <- hist(tanz$longitude[tanz$longitude > 0 ], plot = FALSE )
long_func <- hist(tanz$longitude[tanz$longitude > 0 & tanz$label == 'functional'], plot = FALSE )

barplot(long_func$counts / tot_long$counts, 
        names.arg = long_func$breaks[2:length(long_func$breaks)],
        main = ('% of Wells Functional: longitude > 0'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Longitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# longitude vs functional needs repair

long_func_nr <- hist(tanz$longitude[tanz$longitude > 0 & tanz$label == 'functional needs repair'], breaks = 19, plot = FALSE )

barplot(long_func_nr$counts / tot_long$counts, 
        names.arg = long_func_nr$breaks[2:length(long_func_nr$breaks)],
        main = ('% of Wells Functional Needs Repair: longitude > 0'), ylim = c(0, .5), col =
          'yellow', las = 2, xlab = "Longitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# longitude vs non functional

long_nfunc <- hist(tanz$longitude[tanz$longitude > 0 & tanz$label == 'non functional'], plot = FALSE, breaks = 19 )

barplot(long_nfunc$counts / tot_long$counts, 
        names.arg = long_nfunc$breaks[2:length(long_nfunc$breaks)],
        main = ('% of Wells Non Functional: longitude > 0'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Longitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
# memory cleanup
rm(long_func, long_func_nr, long_nfunc, tot_long)
```
...
```{r, echo = FALSE}
# latitude vs functional

tot_lat <- hist(tanz$latitude[tanz$latitude < -1 ], plot = FALSE )
lat_func <- hist(tanz$latitude[tanz$latitude < -1 & tanz$label == 'functional'], plot = FALSE )

barplot(lat_func$counts / tot_lat$counts, 
        names.arg = lat_func$breaks[2:length(lat_func$breaks)],
        main = ('% of Wells Functional: latitude < -1'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Latitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# latitude vs functional needs repair

lat_func_nr <- hist(tanz$latitude[tanz$latitude < -1 & tanz$label == 'functional needs repair'], breaks = 19, plot = FALSE )

barplot(lat_func_nr$counts / tot_lat$counts, 
        names.arg = lat_func_nr$breaks[2:length(lat_func_nr$breaks)],
        main = ('% of Wells Functional Needs Repair: latitude < -1'), ylim = c(0, .5), col =
          'yellow', las = 2, xlab = "Latitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
```{r, echo = FALSE}
# latitude vs non functional

lat_nfunc <- hist(tanz$latitude[tanz$latitude < -1 & tanz$label == 'non functional'], plot = FALSE, breaks = 19 )

barplot(lat_nfunc$counts / tot_lat$counts, 
        names.arg = lat_nfunc$breaks[2:length(lat_nfunc$breaks)],
        main = ('% of Wells Non Functional: latitude < -1'), ylim = c(0, 1), col =
          'yellow', las = 2, xlab = "Latitude (binned)",
        cex.main=.90, cex.lab=.75, cex.axis=0.75, cex.names=.70 )
...
# memory cleanup
rm(lat_func, lat_func_nr, lat_nfunc, tot_lat)
```
...
Plots for the __basin__ variable show a wide variance in the proportions of functional wells between basins: Three (Lake Nyasa, Pangani, Rufiji) have more than 60% of their wells functioning while the Southern Coast basin has only 37.2% of its wells functioning. In fact, five of the nine basins fail to achieve the overall "functional" metric of 54.3%. The underlying reasons for these disparities should be explored to determine whether or not the underperforming basins can be improved via the application of well management methods/practices being employed within the outperforming basins.
...
Further analysis reveals that values of the __district_code__ variable are not unique across __regions__, as can be seen in the table below where district_code values are aggregated by region. In fact, we find a total of 132 __region__ / __district_code__ combinations within the data set
...
This dependence on the __region__ variable indicates that the __district_code__ variable itself is unlikely to serve as a valid predictor of a well's status. As such, the variable appears unlikely to be useful for predictive modeling purposes.
...
# text(x = 2, y = .9, labels = "0verall = ---", cex = 1)
...
In an attempt to determine whether the lack of a __public_meeting__ value is related to the geographical location of a well, we aggregate the data by __ward__ and calculate the following:

- the total number of wells per ward

- the number of wells within a ward having a __public_meeting__ value of __TRUE__

- the percentage of wells within a ward having a __public_meeting__ value of __TRUE__

- the number of wells within a ward having an unknown __public_meeting__ value

- the percentage of wells within a ward having an uknown __public_meeting__
...
Summary statistics for the percentage of unknown __public_meeting__ values per __ward__ indicate that one or more __ward__'s within the data set have no valid __public_meeting__ values:
...
What this tells us is that if we wish to impute __public_meeting__ values for the data records lacking valid values, we should not attempt to do so solely on the basis of a record's __ward__ value since a significant number of __wards__ contain no valid __public_meeting__ values from which a reasonable imputation for the __public_meeting__ indicator can be derived.

We explore the __lga__ variable using the same aggregation techniques described above for the __ward__ variable,
...
Summary statistics for the percentage of unknown __public_meeting__ values per __lga__ indicate that all __lga__'s within the data set have at least some valid __public_meeting__ values:
...
As such, if we wish to impute __public_meeting__ values for the data records lacking valid values, use of a record's __ward__ and __lga__ values together may allow us to infer whether or not the missing value should be __TRUE__ or __FALSE__ based on the percentage of wells within the __ward__ or __lga__ sharing a __TRUE__ value. For example, if the percentage of __TRUE__ __public_meeting__ values within a __ward__ (or __lga__ if the __ward__ lacks any valid __public_meeting__ values) exceeds 50%, it would seem reasonable to assign a value of __TRUE__ to any missing __public_meeting__ value pertaining to that __ward__ (or __lga__). This approach is applied in the __Data Preparation__ section of this document.
...
Analysis of the __scheme_management__ variable's missing values reveals the following:
...
2. Instances of missing __scheme_management__ values are not correlated in any way with missing __public_meeting__ or __permit__ values, despite the fact that the missing value counts for each of those variables are somewhat similar.

3. Aggregating by geographic variables also fails to produce even a single valid __scheme_management__ value for many __subvillages__ and __wards__. At the __lga__ and __region__ levels we can generally find at least several valid values per __lga__ or __region__, though some __lga__'s contain only a single valid value.

4. Aggregating at the __ward__ level shows that 75 wards lack any valid __scheme_management__ value, and those 75 wards cover 1,413 (36.4%) of the 3,877 missing scheme_management values.

5. At the __lga__ level it appears as though nearly every lga has at least one valid scheme_management value. However, in some lga's the number of unknowns is an order of magnitude greater than the number of known values. For example, in the __lga__ of Kahama there are 736 wells having an unknown __scheme_management__ value and only 100 wells with known values that are spread across several distinct __scheme_management__ categories.

One possible approach to dealing with the missing values would be to assign the most frequently occurring valid __scheme_management__ value found with the corresponding __ward__ or __lga__ as the imputed value. However, that approach seems inappropriate since many wards and lga's are dominated by unknown values: in such instances the most frequently occurring valid value will likely represent a relatively small percentage of the total number of data records for a __ward__ or __lga__, and assigning that value to a missing __scheme_management__ record would likely introduce far too much inappropriate statistical bias within the data set. As such, it may be preferable to simply mark the missing values as "unknown" to prevent the introduction of such bias.

```{r, echo = FALSE, eval = FALSE}
# c_ward contains the total number of wells by ward
scheme_m_tmp <- tanz[, c("id", "ward", "lga", "region", "scheme_management", "extraction_type")]

# fill in NA's so data can be grouped properly
scheme_m_tmp$scheme_management[is.na(scheme_m_tmp$scheme_management)] <- "unknown"

# aggregate the data
scheme_ward_sum <- arrange(summarise(group_by(scheme_m_tmp, ward, scheme_management), 
                               TotalWells = length(unique(id)),
                               SchemeUnk = sum(scheme_management == "unknown") ))

# get unique ward names
scheme_ward <- data.frame(unique(scheme_ward_sum$ward))
colnames(scheme_ward) <- c("ward")

# add a new column for the max managment scheme
scheme_ward$max_ms <- NA

# now find max scheme_managent value for ward
for (i in 1:nrow(scheme_ward)) {
  tmp <- arrange(subset(scheme_ward_sum, ward == scheme_ward$ward[i]), desc(TotalWells))
  
  if (tmp$scheme_management[1] != "unknown") {
    
     scheme_ward$max_ms[i] <- tmp$scheme_management[1]
  } else if (nrow(tmp) > 1) {
    scheme_ward$max_ms[i] <- tmp$scheme_management[2]
    
  } else {
    scheme_ward$max_ms[i] <- "unknown"
  }
    
} # for i

# check count of unknowns = 75 => 75 wards have no known management scheme
sum(scheme_ward$max_ms == "unknown")

# subset to get list of wards having no valid management_scheme value
w_nms <- subset(scheme_ward, max_ms == "unknown")

# subset orig data by w_nms wards
t_wnms <- tanz[which(tanz$ward %in% w_nms$ward),]

# get count of records containing those wards = 1413
length(unique(t_wnms$ward))
...
scheme_lga_sum <- arrange(summarise(group_by(scheme_m_tmp, lga, scheme_management), 
                               TotalWells = length(unique(id)),
                               SchemeUnk = sum(scheme_management == "unknown")
                              #  ), desc(TotalWells) )
))

scheme_reg_sum <- arrange(summarise(group_by(scheme_m_tmp, region, scheme_management), 
                               TotalWells = length(unique(id)),
                               SchemeUnk = sum(scheme_management == "unknown")
                              #  ), desc(TotalWells) )
))
...
In an attempt to determine whether the lack of a __permit__ value is related to the geographical location of a well, we aggregate the data by __ward__ and calculate the following:

- the total number of wells per ward

- the number of wells within a ward having a __permit__ value of __TRUE__

- the percentage of wells within a ward having a __permit__ value of __TRUE__

- the number of wells within a ward having an unknown __permit__ value

- the percentage of wells within a ward having an uknown __permit__
...
Summary statistics for the percentage of unknown __permit__ values per __ward__ indicate that one or more __ward__'s within the data set have no valid __permit__ values:
...
What this tells us is that if we wish to impute __permit__ values for the data records lacking valid values, we should not attempt to do so solely via a record's __ward__ value since a significant number of __wards__ contain no valid __permit__ values from which a reasonable imputation for the __permit__ indicator can be derived.

As an alternative, we explore the __lga__ variable using the same aggregation techniques described above for the __ward__ variable,
...
Summary statistics for the percentage of unknown __permit__ values per __lga__ indicate that one or more __lga__'s within the data set have no valid __permit__ values:
...
In fact, we find a total of 3 such __lga__'s within the data set, as shown below.
...
We now explore the __region__ variable using the same aggregation techniques described above for the __ward__ and __lga__ variables,
...
Summary statistics for the percentage of unknown __permit__ values per __region__ indicate that all __region__'s within the data set have at least some valid __permit__ values:
...
As such, if we wish to impute __permit__ values for the data records lacking valid values, use of a record's __lga__ and __region__ values may allow us to infer whether or not the missing value should be __TRUE__ or __FALSE__ based on the percentage of wells within the __lga__ or __region__ sharing a __TRUE__ value. For example, if the percentage of __TRUE__ __permit__ values within an __lga__ exceeds 50%, it would seem reasonable to assign a value of __TRUE__ to any missing __permit__ value pertaining to that __lga__. However, if the specific __lga__ lacks any valid __permit__ values, we can instead make use of the percentage of __TRUE__ __permit__ values within the associated __region__ and apply the same 50% thresholding approach described for use with the __lga__ variable's values. This approach is applied in the __Data Preparation__ section of this document.
...
More than 53% of the records in the data set contain either missing or invalid data values. Our data exploration efforts allowed us to uncover relationships between various independent variables that could serve as the basis of statistically valid imputation algorithms for the missing values of the `amount_tsh`, `gps_height`, `construction_year`, `latitude`, `longitude`, `public_meeting`, and `permit` variables. A summary of the various imputation algorithms is provided in the table below.

| Variable Name(s)     | Imputation Method                                                 |
|----------------------|-------------------------------------------------------------------|
| amount_tsh           | Median non-zero amount_tsh of corresponding extraction_type_group |
| gps_height           | Linear regression model using latitude + longitude as indep. vars |
| population           | Impute using the median non-zero population value                 |
| latitude & longitude | Mean latitude and longitude values from most granular geographic  |
| (-)                  | indicator having non-zero latitude and longitude values           |
| construction_year    | Median non-zero construction_year of corresponding basin          |
| (-)                  | or extraction_type, whichever is smaller                          |
| public_meeting       | If % TRUE public_meeting values in ward (or lga if no valid       |
| (-)                  | values found in ward) > 50%, assign TRUE, else FALSE              |
| permit               | If % TRUE permit values in ward (or lga or region if no valid     |
| (-)                  | valid values found in ward or lga) > 50%, assign TRUE, else FALSE |
...
__Approach__: Imputation using the mean __latitude__ and __longitude__ values found for the most granular geographic indicator having non-zero latitude and longitude values. 

Our data exploration efforts uncovered 1812 zero values within the __longitude__ variable and 1819 values less than -1 for the __latitude__ variable. Such values were deemed invalid relative to all other __latitude__ and __longitude__ values found within the data set. Fortunately, there are a wide variety of other geographic indicator variables whose categorical average mean values can serve as proxies for the invalid __latitude__ and __longitude__ values.

The __Python__ code shown below checks for invalid __latitude__ and __longitude__ values within each data set record. If an invalid value is found for either one, the mean non-zero __latitude__ and __longitude__ values of the corresponding __ward__, __lga__, or __region__ are assigned to the record as follows:

- If non-zero __ward__ means are found for both variables, those means are assigned to the record;

- If one or both non-zero __ward__ means are not available, the algorithm checks to see whether non-zero __latitude__ and __longitude__ means are available for the record's __lga__ category. If non-zero __lga__ means are found for both variables, those means are assigned to the record;

- If imputation is not feasible at both the __ward__ or __lga__, levels (as described above), the mean non-zero __latitude__ and __longitude__ values for the record's __region__ category are assigned to the record.

The updated __latitude__ and __longitude__ values are then loaded into new variables called __longitude_new__ and __latitude_new__, with the original __latitude__ and __longitude__ variable values remaining unchanged.
...
(Need binning explanation for Sekhar's approach)
...
The __subvillage__ variable contains a relatively small number (371) of missing values. With 19,288 distinct known values/categories, the variable is also by far the most diverse categorical variable within the data set. Some __subvillage__ values appear to be duplicative of a data record's __ward__ variable (e.g., "Madukani") while others are merely a number or alphabetical letter. Given these characteristics, it seems unlikely that a stastically valid imputation scheme can be applied to the missing __subvillage__ values. Therefore, we will simply assign a value of "unknown" to the missing data values found within the variable. This approach is implemented in the __R__ code shown below. Please note that no new variable is created since imputed values are not being applied to the missing data values.

```{r, eval = FALSE}
# assign "unknown" to NA's. No need to create a new variable

# put imputed vals into orig data
tanz$subvillage[is.na(tanz$subvillage)] <- "unknown"
```


### Data Prep: Public Meeting

__Approach__: If the percentage of __TRUE__ __public_meeting__ values within a __ward__ (or __lga__ if the __ward__ contains no valid __public_meeting__ values) exceeds 50%, assign a value of __TRUE__ to the missing value.

Our __Data_Exploration__ work found 3,334 missing __public_meeting__ values within the data set. As part of that analysis we also calculated the percentage of __TRUE__ public meeting values within each __ward__ and __lga__, and we verified that every __lga__ contains at least some valid __public_meeting__ values, whereas many __wards__ contained none. 

The imputation approach applied for the __public_meeting__ variable thereby makes use of a simple thresholding approach, whereby a missing value is assigned a __TRUE__ or __FALSE__ value relative to the percentage of __TRUE__ public meeting values within the __ward__ ( or __lga__ if the __ward__ lacks a valid __public_meeting__ value) indicated for the data record in question. If that percentage exceeds 50%, a __TRUE__ value is imputed for the missing data value, otherwise a __FALSE__ value is imputed. The __R__ code shown below implements this approach, with a new variable ( __public_meeting_new__ ) being added to the data set to house the modified __public_meeting__ values.
...
pmeet_sub <- subset(tanz, is.na(public_meeting))

# add new variable to house imputed values
tanz$public_meeting_new <- tanz$public_meeting
...
# put imputed vals into orig data
tanz$public_meeting_new[is.na(tanz$public_meeting_new)] <- pmeet_sub$public_meeting

######################################
# sanity check - make sure no NA's remain
# sum(is.na(tanz$public_meeting_new))
...
Our data exploration work showed that imputation for missing __scheme_management__ values is unlikely to be effective. Therefore, we will simply assign a value of "unknown" to the missing data values found within the variable. This approach is implemented in the __R__ code shown below. Please note that no new variable is created since imputed values are not being applied to the missing data values.

```{r, eval = FALSE}
# assign "unknown" to NA's. No need to create a new variable

# put imputed vals into orig data
tanz$scheme_management[is.na(tanz$scheme_management)] <- "unknown"
```


### Data Prep: scheme_name

With 2700 identifiable distinct values and 28,166 of the 54,900 records having missing values for the variable, __scheme_name__ appears to be a poor candidate for an effective and statistically valid imputation approach. There appears to be no clear relationship between the known __scheme_name__ values and other variable's values, and the scheme names themselves seem likely to have been assigned/constructed without the use of an overriding common set of naming guidelines. Therefore, we will simply assign a value of "unknown" to the missing data values found within the variable. This approach is implemented in the __R__ code shown below. Please note that no new variable is created since imputed values are not being applied to the missing data values.

```{r, eval = FALSE}
# assign "unknown" to NA's. No need to create a new variable

# put imputed vals into orig data
tanz$scheme_name[is.na(tanz$scheme_name)] <- "unknown"
```


### Data Prep: Permit

__Approach__: If the percentage of __TRUE__ __permit__ values within a __ward__ (or __lga__ or __region__ if the __ward__ or __lga__ contains no valid __permit__ values) exceeds 50%, assign a value of __TRUE__ to the missing value.

Our __Data_Exploration__ work found 3,056 missing __permit__ values within the data set. As part of that analysis we also calculated the percentage of __TRUE__ public meeting values within each __ward__, __lga__, and __region__, and we verified that every __region__ contains at least some valid __permit__ values, whereas many __wards__ and three __lga__'s contained none. 

The imputation approach applied for the __permit__ variable thereby makes use of a simple thresholding approach, whereby a missing value is assigned a __TRUE__ or __FALSE__ value relative to the percentage of __TRUE__ __permit__ values within the __ward__  (or __lga__ or __region__ if the __ward__ or __lga__ lacks any valid __permit__ values) indicated for the data record in question. If that percentage exceeds 50%, a __TRUE__ value is imputed for the missing data value, otherwise a __FALSE__ value is imputed. The __R__ code shown below implements this approach, with a new variable ( __permit_new__ ) being added to the data set to house the modified __permit__ values.
...
permit_sub <- subset(tanz, is.na(permit))

# add new variable to house imputed values
tanz$permit_new <- tanz$permit
...
# put imputed vals into orig data
tanz$permit_new[is.na(tanz$permit_new)] <- permit_sub$permit

######################################
# sanity check
sum(is.na(tanz$permit_new))

# memory cleanup
rm(lga_perm_true, reg_perm_true, permit_sub)
```


### Data Prep: construction_year

__Approach__: Impute missing values using median non-zero __construction_year__ values found for __extraction_type__ and __basin__ categories assigned to a data record. Find both median values and assign the lesser of the two as the imputed __construction_year__ value. This approach has the benefit of largely maintaining the original distribution of the non-zero values found within the variable.

Our data exploration work identified 20,709 missing __construction_year__ values within the data set. As we alluded to in the analysis of the __amount_tsh__ variable, other variables within the data set (e.g., __extraction_type__, __extraction_type_group__, __waterpoint_type__, etc.) are at least somewhat indicative of the type of well deployed at each waterpoint. Assuming similar types of wells were generally installed within similar chronological timeframes, it seems reasonable to conclude that use of the median non-zero __construction_year__ value found for similar types of wells may be an effective manner of filling the missing data values.

The __R__ code shown below implements this approach while also ensuring that the selected median __construction_year__ value does not exceed the year value of the data record's __date_recorded__ variable. Logically, an imputed __construction_year__ value should not exceed the year value during which the data was recorded for a given well.
...
### Adding a New Numeric Variable: "pump_age"

The data records provided in the data set appear to have (for the most part) recorded during the 2011 - 2013 timeframe. Therefore, the age of the observations provided within the data set varies from record to record, with some observations being older (e.g., those from 2011) than those which were recorded later (e.g., those from 2013). In fact, the __date_recorded__ allows us to estimate the age of a pump by subtracting a data record's __construction_year__ value from the __date_recorded__ variable's 'year' component. 

Since the age of a pump may prove to be predictive of its status (i.e., 'functional', etc.), we add a new __pump_age__ variable to the data set via the subtract logic explained above. The __R__ code shown below implements this logic while also ensuring that all wells have an age of at least one year. 

```{r, eval = FALSE}
# create new variable: well_age = year date recorded - construction_year (including imputed values)

# get year value from date_recorded
tmp_drec <- year(as.Date(tanz$date_recorded, format = "%m/%d/%Y"))

# subtract new construction year values (imputations) from date recorded 'year' value
tanz$pump_age <- tmp_drec - tanz$cons_yr_new

# set any zero values to 1: new wells are considered to be 1 year old due to uncertainty
tanz$pump_age[tanz$pump_age < 1] <- 1
...
## Create Training + Evaluation Subsets
...
set.seed(123)
eval.d <- tanz[sample(nrow(tanz), 20000), ]

eval.d <- arrange(eval.d, id)

# create TRAINING data set: remove all eval.d observations from master data set
training.d <- tanz[-eval.d$id, ]

# now write both files to disc
write.csv(eval.d, file = "C:/data/698/FP-EVAL-DATASET.csv", row.names = FALSE)
write.csv(training.d, file = "C:/data/698/FP-TRAINING-DATASET.csv", row.names = FALSE)

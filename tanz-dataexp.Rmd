...
Analysis of the __amount_tsh = 0__ values relative to various geographic variables provided within the data set reveals a significant amount of clustering of such values by subvillage, ward, lga, district_code, and region. The table below provides a summary of this clustering.
...
Further analysis is required to determine whether the remaining zero values are in fact valid data values or if they merely represent the lack of an accurate total static head measurement for the associated wells. However, given the widespread lack of accurate measurements found for these geographic areas it seems highly likely that the zero values are simply invalid.
...
etc_mtsh <- arrange(summarise(group_by(t_atsh, extraction_type_class), 
...
Since each well represented within the data set has known values for both of these variables, the median values shown in the tables above may allow us to impute a reasonable value for each instance of __amount_tsh = 0__ within the data set.
...
Analysis of individual regions reveals that the four regions of Dodoma, Kagera, Mbeya and Tabora all appear to have no non-zero values for the following variables:
...
As shown below, these 4 regions comprise 12,115 of the 59,400 records in the data set (20.39%), including 514 of the unique wards and 4644 of the unique subvillages.  This lack of non-zero values makes it highly unlikely that we will be able to derive imputed values for the zero values of the five variables listed above using the geographical indicators provided within the data set.
...
Imputation based on mean of sum of median __amount_tsh__ values found for corresponding __extraction_type_class__ and __waterpoint_type_group__ values.


### gps_height

Approach: A linear model using latitude + longitude as independent variables and gps_height as the dependent variable.
...
Exclusion of such data records from our imputation work will ensure that the remaining nearly 19,000 zero values within the __gps_height__ variable will be updated with an imputed altitude value that reflect the longitude and latitude coordinates of the corresponding well.
...
Impute using the median non-zero value
...
20,709 NA's. No way to impute based on geographic variables due to many regions, districts, lga's, wards, and subvillages have no non-zero construction year values. May be forced to impute using a median of the non-zero values though such an approach may prove problematic to due large percentage of data values that require imputation. Alternative is to use a linear model. 
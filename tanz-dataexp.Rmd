...
-------------------------------| ------------------------------------------------------------------
`id`                    | num  | Identification Variable (not a predictor)   
`date_recorded`         | date | Date of data collection by survey company
`recorded_by`           | char | Name of the data collection / survey company
`amount_tsh      `      | num  | Metric indicating total static head for pump; should be > 0
`gps_height`            | num  | Altitude of pump
`population`            | num  | Human population in immediate vicinty of pump
`longitude`             | num  | Longitudinal coordinate of pump
`latitude`              | num  | Latitudinal coordinate of pump
`num_private`           | num  | No definition is available for this variable
`funder`                | char | Name of organization that funded installation of pump
`installer`             | char | Name of organization that installed the pump
`wpt_name`              | char | Name assigned to given waterpoint
`basin`                 | char | Name of geographic water basin where pump is located
`subvillage`            | char | Name of geographic subvillage where pump is located
`region`                | char | Name of geographic region in Tanzania where pump is located
`region_code`           | num  | Numeric ID for 'region' variable
`district_code`         | num  | Numeric ID of district within a region where pump is located
`lga`                   | char | Tanzania-specific geographic indicator of where pump is located
`ward`                  | char | Name of Tanzanian geographic ward where pump is located
`public_meeting`        | char | True/False indicator
`scheme_management`     | char | Type of the organization responsible for management of pump
`scheme_name`           | char | Name of organization responsible for management of pump
`permit`                | char | True/False indicating whether the pump has valid permit
`construction_year`     | num  | The year the pump was installed
`extraction_type`       | char | Method of extraction used at a given pump site
`extraction_type_group` | char | Aggregation of extraction_type categories
`extraction_type_class` | char | Aggregation of extraction_type_group categories
`management`            | char | Name of method employed for management of a given pump
`management_group`      | char | Possibly an aggregation of management categories
`payment`               | char | Categorical indicator of payment method required of pump users
`payment_type`          | char | Appears to be a duplicate of payment categories
`water_quality`         | char | Categorical indicator of water quality produced by pump
`quality_group`         | char | Aggregation of water_quality categories
`quantity`              | char | Categorical indicator of water quantity produced by pump
`quantity_group`        | char | Aggregation of quantity categories
`source`                | char | Type of source of the water for a given pump
`source_type`           | char | Aggregation of source categories
`source_type_class`     | char | Aggregation of source_type categories
`waterpoint_type`       | char | The type of pump installed at a well site
`waterpoint_type_group` | char | Aggregation of waterpoint_type categories

...
Data exploration work showed that imputation for missing __scheme_management__ values was unlikely to be effective. Therefore, we will simply a value of "unknown" to the missing data values found within the variable. This approach is implemented in the __R__ code shown below. Please note that no new variable is created since imputed values are not being applied to the missing data values.
...
With 2700 identifiable distinct values and 28,166 of the 54,900 records having missing values for the variable, __scheme_name__ appears to be a poor candidate for an effective and statistically valid imputation approach. There appears to be no clear relationship between the known __scheme_name__ values and other variable's values, and the scheme names themselves seem likely to have been assigned/constructed without the use of an overriding common set of naming guidelines. Therefore, we will simply a value of "unknown" to the missing data values found within the variable. This approach is implemented in the __R__ code shown below. Please note that no new variable is created since imputed values are not being applied to the missing data values.
